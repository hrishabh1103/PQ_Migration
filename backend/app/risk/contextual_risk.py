...
class RiskContext(BaseModel):
    ...
    network_exposure: Optional[str] = "INTERNET" # INTERNET, INTERNAL, ISOLATED
    asset_criticality: Optional[str] = "HIGH" # HIGH, MEDIUM, LOW
    data_classification: Optional[str] = "CONFIDENTIAL" # RESTRICTED, CONFIDENTIAL, PUBLIC
    confidentiality_lifetime: Optional[str] = "10_YEARS" # >10_YEARS, 5-10_YEARS, <5_YEARS
    regulatory_context: Optional[str] = "CNSA_2.0" # CNSA_2.0, FIPS_203_204_205, PCI_DSS, GENERAL
    migration_deadline: Optional[str] = "2030"
...
class ContextualRiskEngine:
    ...
    def evaluate(cls, ctx: RiskContext) -> ContextualRiskEvaluation:
        ...
        # 1. Evaluate Algorithm Primitive & Quantum Vulnerability
        ...
        if is_rsa or is_ecdsa or is_ecdh:
            base_score = 8.5
            factors.append(f"Algorithm '{ctx.algorithm}' is vulnerable to Shor's polynomial-time quantum algorithm.")
            ...
            if ctx.purpose in [PurposeType.SIGNATURE, PurposeType.AUTHENTICATION]:
                ...
                factors.append("Purpose is SIGNATURE/AUTHENTICATION: Recommended replacement is ML-DSA-65 or SLH-DSA (FIPS 204/205).")
                ...
                factors.append("Purpose is KEY_ESTABLISHMENT: Recommended replacement is ML-KEM-768 (FIPS 203).")
            else:
                if is_rsa:
                ...
                factors.append(f"Purpose is {ctx.purpose.value}: Assessed general asymmetric quantum vulnerability.")
            ...
            factors.append("Hash algorithm exhibits classical collision weakness under differential cryptanalysis.")
            ...
            factors.append("Symmetric key length < 256 bits offers reduced quantum security against Grover search.")
            ...
            timeline = "Compliant"
            factors.append("Algorithm aligns with standard symmetric or PQC candidate taxonomies.")

        # 2. Contextual Score Weightings
        if ctx.network_exposure == "INTERNET":
            base_score += 1.0
            factors.append("Network Exposure is INTERNET (+1.0 Risk Weight).")
        elif ctx.network_exposure == "INTERNAL":
            factors.append("Network Exposure is INTERNAL.")

        if ctx.data_classification in ["RESTRICTED", "CONFIDENTIAL"]:
            base_score += 0.5
            factors.append(f"Data Classification is {ctx.data_classification} (+0.5 Risk Weight).")

        if ctx.asset_criticality == "HIGH":
            base_score += 0.5
            factors.append("Asset Criticality is HIGH (+0.5 Risk Weight).")
